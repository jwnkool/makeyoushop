 ...
 * Index of function :
 -----------------------------------------------------------------
 	1. Function checkout index
	2. Function checkout billing address
	3. Function checkout shipping address
	4. Function checkout check ongkir
	5. Function checkout shipping method
	6. Function checkout shipping form
	7. Function checkout get data from API RajaOngkir
		7.1 Function get json data province from API RajaOngkir
		7.2 Function get json data city from API RajaOngkir
		7.3 Function get cost total from API RajaOngkir
		7.4 Function pilih ongkir
	8. Function checkout validate shipping option
	9. Function checkout get shipping method
   10. Function checkout payment method 
   11. Function checkout payment form 
   12. Function checkout payment validate 
   13. Function checkout get payment methods
   14. Function checkout confirm
   15. Function checkout login
   16. Function checkout register
   17. Function checkout place order detail & send to mail customer
 =================================================================
*/
...
class Checkout extends Front_Controller {
	...
	function step_1()
	{
		...
		else
		{
			...
			if($this->input->post('use_shipping')=='yes')
			{
				...
				redirect('checkout/step_cekongkir');
			} else {
	...
	/*===============================================================
	 4. Function checkout check ongkir
 	=================================================================*/
	function step_cekongkir()
	{
	
		$data['customer']			= $this->go_cart->customer();
		
		$this->view('checkout/cekongkir',$data);
	...
	/*===============================================================
 	...
 	=================================================================*/
		...
		function cost(){
			...
			$cost = $this->rajaongkir->cost($destination,$weight,$courier);
			/* parse json data & view your cost*/
					...
					foreach($r->costs as $rc):
						foreach($rc->cost as $rcc):
							echo '<tr>
									...
									<td>';?>
									<?php echo form_open('checkout/pilihongkir'); ?>
									  <input type="hidden" name="harga" value="<?php echo $rcc->value;?>"> 
									  <input type="hidden" name="paket" value="<b><?php echo $r->code;?></b>, <?php echo $rc->service;?> (<?php echo $rc->description;?>)<br />code : <?php echo $destination = $_GET['destination'];?>"> 
									  <center><input type="submit" name="submit" value="Check" class="btn btn-md btn-warning"></center> 
									</form>
									<?php echo '</td>
								</tr>';
						endforeach;
		...
		function pilihongkir(){
			/* check ongkir service & saving to table setting in database */
			$this->Settings_model->getPilihongkir();
	...
	{		
		...
		if( ! empty($order_downloads))
		{
			...
			if(!empty($data['customer']['id']))
			{
				...
				$download_section = str_replace('{download_link}', anchor(site_url('secure/my_downloads'), lang('download_link')),$downlod_msg_record['content_messages']);
			} else {
				// non regs will receive a code
				$download_section = str_replace('{download_link}', anchor(site_url('secure/my_downloads').'/'.$order_downloads['code'], lang('download_link')), $downlod_msg_record['content_messages']);
			}
		}
		
		$row['content_messages'] = html_entity_decode($row['content_messages']);
		
		// set replacement values for subject & body
		// {customer_name}
		$row['subject'] = str_replace('{customer_name}', $data['customer']['firstname'].' '.$data['customer']['lastname'], $row['subject']);
		$row['content_messages'] = str_replace('{customer_name}', $data['customer']['firstname'].' '.$data['customer']['lastname'], $row['content_messages']);
		
		// {url}
		$row['subject'] = str_replace('{url}', $this->config->item('base_url'), $row['subject']);
		$row['content_messages'] = str_replace('{url}', $this->config->item('base_url'), $row['content_messages']);
		
		// {site_name}
		$row['subject'] = str_replace('{site_name}', $this->config->item('company_name'), $row['subject']);
		$row['content_messages'] = str_replace('{site_name}', $this->config->item('company_name'), $row['content_messages']);
			
		// {order_summary}
		$row['content_messages'] = str_replace('{order_summary}', $this->load->view('order_email', $data, true), $row['content_messages']);
		
		// {download_section}
		$row['content_messages'] = str_replace('{download_section}', $download_section, $row['content_messages']);
		...
		$this->email->message($row['content_messages']);
