 ...
 * Index of function :
 -----------------------------------------------------------------
 	1. Function orders home
	2. Function orders export to xml
	3. Function orders details
	4. Function packing slip format
	5. Function edit order status
	6. Function send email notification order to customer
	7. Function orders delete
 =================================================================
*/

class Backend_orders extends Admin_Controller {	
	...
	function index($sort_by='order_number',$sort_order='desc', $code=0, $page=0, $rows=15)
	{
		...
		if($this->input->post('submit') == 'export')
		{
			...
			force_download_content('orders.xml', $this->load->view('orders_xml', $data, true));
		...
		$config['base_url']			= site_url('backend_orders/index/'.$sort_by.'/'.$sort_order.'/'.$code.'/');
	...
	function export()
	{
		...
		force_download_content('orders.xml', 
		$this->load->view('orders_xml', $data, true));
	...
	function order($id)
	{
		...
		******************************/
    	...
    	{
 			// fix html
 			$msg['content_messages'] = str_replace("\n", '', html_entity_decode($msg['content_messages']));
 			
 			// {order_number}
 			$msg['subject'] = str_replace('{order_number}', $data['order']->order_number, $msg['subject']);
			$msg['content_messages'] = str_replace('{order_number}', $data['order']->order_number, $msg['content_messages']);
    		
    		// {url}
			$msg['subject'] = str_replace('{url}', $this->config->item('base_url'), $msg['subject']);
			$msg['content_messages'] = str_replace('{url}', $this->config->item('base_url'), $msg['content_messages']);
			
			// {site_name}
			$msg['subject'] = str_replace('{site_name}', $this->config->item('company_name'), $msg['subject']);
			$msg['content_messages'] = str_replace('{site_name}', $this->config->item('company_name'), $msg['content_messages']);
		...
		foreach($data['order']->contents as $orderkey=>$product)
		{
			if(isset($product['is_gc']) && (bool)$product['is_gc'])
			{
				if($this->Gift_card_model->is_active($product['sku']))
				{
					...
					$data['order']->contents[$orderkey]['gc_status'] = ' [ <a href="'. base_url() . 'backend_giftcards/activate/'. $product['code'].'">'.lang('activate').'</a> ]';
				}
			}
		}
	...
	function packing_slip($order_id)
	{
		...
		$this->load->view('packing_slip', $data);
	}
    ...
    function send_notification($order_id='')
    {
		...
		$this->email->message(html_entity_decode($this->input->post('content_messages')));
		
		$this->email->send();
		
		$this->session->set_flashdata('message', lang('sent_notification_message'));
		redirect('backend_orders/order/'.$order_id);
	}
