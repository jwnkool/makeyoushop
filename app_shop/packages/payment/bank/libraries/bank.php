...
class bank
{
	...
	function __construct()
	{
		...
		$this->CI->lang->load('bank');
		
		$this->method_name	= lang('bank_transfer');
	}
	...
	*/
	...
	{
		$settings	= $this->CI->Settings_model->get_settings('bank');
	...
	function install()
	{
		...
		$this->CI->Settings_model->save_settings('bank', array('enabled'=>'0'));
	}
	
	function uninstall()
	{
		$this->CI->Settings_model->delete_settings('bank');
	}
	...
	{	
		...
		else
		{
			...
			$this->CI->Settings_model->save_settings('bank', array('enabled'=>$_POST['enabled']));
